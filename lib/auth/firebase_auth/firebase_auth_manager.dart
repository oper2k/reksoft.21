...
class FirebaseAuthManager extends AuthManager
  ...
  Future resetPassword({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Письмо отправлено на почту')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : Reksoft21FirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => 'Пользователь с таким емейлом уже существует',
        'INVALID_LOGIN_CREDENTIALS' => 'Неверный логин или пароль',
        _ => 'Ошибка: [error]'.replaceAll('[error]', e.message!),
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
